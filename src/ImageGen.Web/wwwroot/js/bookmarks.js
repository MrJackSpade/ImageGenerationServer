...
(function () {
  document.addEventListener("click", async (e) => {
    ...
    if (pin) {
      ...
      try {
        ...
        location.reload();
      } catch (e) { console.error("bookmark pin update failed:", e); toast("Couldn't update pin"); }
      return;
    }
    ...
    catch (e) { console.error("bookmark remove failed:", e); toast("Couldn't remove"); }
  });
  ...
  document.addEventListener("click", (e) => {
    ...
    saveBookmarkPrefs(JSON.stringify({ collapsed: [...collapsed] })).catch(e => { console.error("bookmark prefs save failed:", e); toast("Couldn't save that"); });
  });
  ...
  (async () => {
    ...
    let s; try { s = await fetchSettings(); } catch (e) { console.error("bookmark settings load failed:", e); return; }
    if (s && s.bookmarkPrefs) {
      try { collapsed = new Set(JSON.parse(s.bookmarkPrefs).collapsed || []); }
      catch (e) { console.error("bookmark prefs unreadable:", e); return; }   // don't overwrite the stored blob with a guess
    }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll("[data-image-record]").forEach(el => {
      let rec; try { rec = JSON.parse(el.dataset.imageRecord); } catch (e) { console.error("bookmark image record unreadable:", e); return; }
      window.attachCategoryLongPress(el, () => ({
