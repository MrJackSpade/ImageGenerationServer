...
/// <summary>
/// This machine's recent per-model average render timings (ms), keyed by config id (#200) — the decoration behind the
/// <c>/forge/workflows</c> ETAs and the <c>/forge/queue</c> header ETA. Flushed exactly on job finalization (the only
/// event that changes the averages), with a 5-minute backstop.
///
