...
public sealed class LoraUserSettingRepository(IDbConnectionFactory connectionFactory, IUserCipher cipher) : ILoraUserSettingRepository
{
    ...
    {
        Dictionary<string, LoraUserSetting> result = new Dictionary<string, LoraUserSetting>(StringComparer.OrdinalIgnoreCase);
        if (loraNames.Count == 0) return result;

        List<string> names = loraNames.ToList();
        string[] ps = new string[names.Count];
        for (int i = 0; i < names.Count; i++) ps[i] = "@a" + i;
        ...
        Dictionary<string, string> byCipher = new Dictionary<string, string>(StringComparer.Ordinal);
        ...
        List<(string Enc, string? Tw, bool Aa)> raw = new List<(string Enc, string? Tw, bool Aa)>();
