...
internal static class CategoryIo
{
    ...
    {
        Dictionary<long, List<string>> byParent = [];
        ...
        List<(long ParentId, string Category)> raw = [];
        ...
        IReadOnlyList<string> categories, long userId, IUserCipher cipher, CancellationToken ct)
    {
        ...
        {
            _ = del.AddParam("@parent", parentId);
        ...
        foreach (string raw in categories)
        {
            ...
            _ = cmd.AddParam("@category", await cipher.DeterministicAsync(userId, name, ct));
