...
public sealed class Ideogram4Workflow : Txt2ImgWorkflowBase
{
    ...
    public override Dictionary<string, object> Build(ParamValues p, ResolvedRequirements req, WorkflowInputs inputs)
    {
        ...
        wf["20"] = ComfyGraph.Node("CLIPLoader", new { clip_name = req.TextEncoder(0), type = "ideogram4", device = "default" });
        ...
        wf["2"]  = ComfyGraph.Node("CFGOverride", new { model = ComfyGraph.Ref("4", 0), cfg = p.DblReq("cfg_override"), start_percent = 0.7, end_percent = 1.0 });
        wf["22"] = ComfyGraph.Node("DualModelGuider", new { model = ComfyGraph.Ref("2", 0), positive = ComfyGraph.Ref("6", 0), model_negative = ComfyGraph.Ref("40", 0), negative = ComfyGraph.Ref("26", 0), cfg = p.DblReq("cfg") });

        wf["5"]  = ComfyGraph.Node("EmptyFlux2LatentImage", new { width = w, height = h, batch_size = 1 });
        wf["17"] = ComfyGraph.Node("Ideogram4Scheduler", new { steps = p.IntReq("steps"), width = w, height = h, mu = p.DblReq("mu"), std = p.DblReq("std") });
