...
/// model (<c>ModelSamplingAuraFlow</c>+<c>CFGNorm</c> unless AIO bakes its own sampling), the bucket-snapped source
/// image (the encode's <c>image1</c>, reused as the fill <c>pixels</c> on the inpaint path), that image's VAE latent
/// (the default sample latent on the plain-edit path), and the VAE edge.</summary>
...
/// model/CLIP/VAE + source <c>LoadImage</c>: the default-resolution normalisation (<c>FluxKontextImageScale</c> snaps
/// to a Qwen-trained bucket + the danamir blur fix), the reference image loads into <c>image2</c>/<c>image3</c>, and
/// the positive/negative <c>TextEncodeQwenImageEditPlus</c> encodes with their reference-latent stitch.
///
...
/// this head diverges: the plain editor samples the source latent directly; the inpaint editor routes it through
/// <c>InpaintModelConditioning</c> with a painted mask.</para>
/// </summary>
internal static class QwenReferenceHead
{
    ...
    /// the source <c>LoadImage</c> at <see cref="EditNodes.Source"/>) and passes those edges in. <paramref name="aio"/>
        ...
        WorkflowInputs inputs, string[]? referenceInputs, int? referenceMax, string referenceLatentsMethod)
    {
        ...
        return new QwenRefHeadOut(cond, negCond, ksModel, kontext, VAEEncode.Out(Nodes.SourceEncode), vae0);
    }
