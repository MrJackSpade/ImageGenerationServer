...
public sealed class AnimateDiffSd15Workflow : EditWorkflowBase
{
    ...
    public override Dictionary<string, object> Build(ParamValues p, ResolvedRequirements req, WorkflowInputs inputs)
    {
        ...
        double budgetMp = (p.Int("width") > 0 && p.Int("height") > 0) ? (p.Int("width") * (double)p.Int("height")) / 1_000_000.0 : 0.26;
        string mm = p.Model("motion_model");
        // SD1.5 AnimateDiff REQUIRES sqrt_linear; autoselect picking 'linear' is what makes the motion look horrible.
        string beta = string.IsNullOrWhiteSpace(p.Str("beta_schedule")) ? "sqrt_linear (AnimateDiff)" : p.Str("beta_schedule")!;
        ...
        wf["3"] = ComfyGraph.Node("KSampler", new { seed, steps = p.Int("steps", 20), cfg = p.Dbl("cfg", 1), sampler_name = ComfyGraph.MapSampler(p.Str("sampler")), scheduler = ComfyGraph.MapScheduler(p.Str("scheduler")), denoise = 1.0, model = ComfyGraph.Ref("22", 0), positive = ComfyGraph.Ref("25", 0), negative = ComfyGraph.Ref("25", 1), latent_image = ComfyGraph.Ref("7", 0) });
