...
public abstract class Flux2KleinEditBase : EditWorkflowBase
{
    public override Dictionary<string, object> Build(ParamValues p, ResolvedRequirements req, WorkflowInputs inputs)
    {
        ...
        wf["14"] = ComfyGraph.Node("FluxGuidance", new { conditioning = ComfyGraph.Ref("13", 0), guidance = p.DblOrNull("guidance") ?? 4.0 });
        ...
        wf["29"] = ComfyGraph.Node("Flux2Scheduler", new { steps = p.Int("steps", 20), width = ComfyGraph.Ref("17", 0), height = ComfyGraph.Ref("17", 1) });
