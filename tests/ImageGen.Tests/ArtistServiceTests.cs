...
public sealed class ArtistServiceTests(TestDatabaseFixture fixture)
{
    ...
    {
        var user = await fixture.NewUserAsync("artist-display");
        ...
        var fallback = await svc.GetDisplayAsync(user.Id, "monet", Ct);
        ...
        var chosen = await svc.GetDisplayAsync(user.Id, "monet", Ct);
    ...
    {
        var user = await fixture.NewUserAsync("artist-reject");
    ...
    {
        var alice = await fixture.NewUserAsync("artist-many-a");
        var bob = await fixture.NewUserAsync("artist-many-b");
        ...
        var resolved = await svc.ResolveManyAsync(alice.Id, ["monet", "van_gogh", "unseen"], Ct);
