...
/// ordinal), Comfy <c>{a|b}</c> choices + <c>{{a|b}}</c> fan-out → resolved combos, and the per-model
...
public sealed class TagCompilerTests
{
    ...
    private static readonly string[] Colors = ["red", "blue"];
    private static readonly string[] EditColors = ["make it red", "make it blue"];
    ...
    [Fact]
    public void Legacy_square_choice_remains_a_random_alias() =>
        Assert.Equal("y", Assert.Single(TagGroup.Parse("[x|y]").Generate(_ => 1)).RawResolved);
    ...
    [InlineData("{solo}")]        // no '|' — not a choice group
    ...
    [InlineData(@"\[red|blue\]", "[red|blue]")]
    [InlineData("{{plain}}", "{{plain}}")] // no top-level pipe: not an exhaustive group
    public void Escaped_or_non_alternating_delimiters_are_literal(string raw, string expected)
    {
        GeneratedTagGroup g = Assert.Single(TagGroup.Parse(raw).Generate());
        Assert.Equal(expected, g.RawResolved);
    }
    ...
    {
        // The whole pipeline: '{{a|b}}' fans out, each combo renders as the image model would see it.
    ...
    {
        IReadOnlyList<RenderItem> items = RenderOrchestrator.ExpandPromptGroups(
    ...
    [Fact]
    public void Enqueue_resolves_a_Comfy_choice_to_one_generate_item()
    {
        IReadOnlyList<RenderItem> items = RenderOrchestrator.ExpandPromptGroups(
            [RenderItem.ForGenerate(new GenerateSpec("wf", "{red|blue}", null, "square"))]);

        GenerateSpec resolved = Assert.IsType<GenerateSpec>(Assert.Single(items).Gen);
        Assert.Contains(resolved.Prompt, Colors);
    }
    ...
    public void Enqueue_leaves_a_group_free_generate_item_untouched()
    {
        ...
        Assert.Same(one, Assert.Single(RenderOrchestrator.ExpandPromptGroups([one])));
    }

    [Fact]
    public void Enqueue_does_not_reinterpret_an_already_resolved_literal_choice()
    {
        RenderItem one = RenderItem.ForGenerate(new GenerateSpec(
            "wf", "literal {red|blue}", null, "square", ResolvePromptSyntax: false));

        Assert.Same(one, Assert.Single(RenderOrchestrator.ExpandPromptGroups([one])));
    }

    [Fact]
    public void Enqueue_resolves_Comfy_choices_for_edit_workflows()
    {
        RenderItem edit = RenderItem.ForEdit(new EditSpec("wf", "make it {red|blue}", "img1"));
        EditSpec resolved = Assert.IsType<EditSpec>(Assert.Single(RenderOrchestrator.ExpandPromptGroups([edit])).Edit);
        Assert.Contains(resolved.Instruction, EditColors);
    }

    [Fact]
    public void Enqueue_fans_an_exhaustive_edit_instruction_into_multiple_slots()
    {
        RenderItem edit = RenderItem.ForEdit(new EditSpec("wf", "make it {{red|blue}}", "img1"));
        IReadOnlyList<RenderItem> items = RenderOrchestrator.ExpandPromptGroups([edit]);

        Assert.Equal(["make it red", "make it blue"], items.Select(i => i.Edit?.Instruction));
    }
