...
**103 models, 147 presets.** Generated from `configurations/workflows/` by `tools/gen-models-doc.py` — edit the catalogue, not this file.
...
| **Boogu-Image Base** | Brand-new (0.1) Apache-2.0 text-to-image foundation model. | [Hugging Face](https://huggingface.co/Comfy-Org/Boogu-Image) |
...
| **Chroma1-Radiance (pixel-space)** | Chroma, with the VAE removed: denoises RGB directly, so there is no latent round-trip and no VAE compression artifacts. | [Hugging Face](https://huggingface.co/Comfy-Org/Chroma1-Radiance_Repackaged) |
...
| **PixelDiT 1300M (pixel-space)** | NVIDIA PixelDiT-1300M. | [Hugging Face](https://huggingface.co/Comfy-Org/PixelDiT) |
...
| Model | What it is | Download |
| --- | --- | --- |
| **Anima** 🏷 | Reinterpret an entire existing image through Anima using a prompt — the source is used as the init latent ('the noise thing') and re-sampled at a partial denoise with NO mask, so the composition is… | [Hugging Face](https://huggingface.co/circlestone-labs/Anima) |
| **Anima (inpaint)** 🏷 | Regenerate a PAINTED region of an existing image with the Anima model — paint the area (e.g. | [Hugging Face](https://huggingface.co/circlestone-labs/Anima) |
| **Anima (outpaint)** 🏷 | Extend an image beyond its edges with Anima. | [Hugging Face](https://huggingface.co/circlestone-labs/Anima) |
...
| **Boogu-Image Edit** | Brand-new (0.1) Apache-2.0 unified edit model. | [Hugging Face](https://huggingface.co/Comfy-Org/Boogu-Image) |
...
| **Chroma1-Radiance (pixel-space)** | Reinterpret an entire existing image through Chroma1-Radiance using a prompt — the source is used as the init latent and re-sampled at a partial denoise with NO mask, so the composition is kept but… | [Hugging Face](https://huggingface.co/Comfy-Org/Chroma1-Radiance_Repackaged) |
...
| **FLUX.1 Fill (inpaint)** | Replace a painted region with the model BUILT for inpainting rather than a txt2img base steered by a ControlNet. | [Hugging Face](https://huggingface.co/black-forest-labs/FLUX.1-Fill-dev) |
| **FLUX.1 Fill (outpaint)** | Extend an image past its edges with the model trained for outpainting. | [Hugging Face](https://huggingface.co/black-forest-labs/FLUX.1-Fill-dev) |
...
| **Qwen-Image (inpaint)** | Regenerate a PAINTED region of an image with base Qwen-Image driven by the InstantX inpainting ControlNet. | [Hugging Face](https://huggingface.co/Comfy-Org/Qwen-Image_ComfyUI) |
| **Qwen-Image (outpaint)** | Extend an image past its edges with base Qwen-Image. | [Hugging Face](https://huggingface.co/Comfy-Org/Qwen-Image_ComfyUI) |
...
| **ChronoEdit-14B** | NVIDIA's temporal-reasoning editor on a Wan 14B backbone (same class as your Wan 2.2). | [Hugging Face](https://huggingface.co/nvidia/ChronoEdit-14B-Diffusers) |
...
| **LTX Video** | Pixel-art image-to-video (locked-palette per-frame quantize). | [Hugging Face](https://huggingface.co/Lightricks/LTX-Video) |
| **LTX Video 13B** | Pixel-art image-to-video (locked-palette per-frame quantize). | [Hugging Face](https://huggingface.co/Lightricks/LTX-Video) |
| **LTX-2** | LTX-2 image-to-video with the deterministic pixel-art quantizer applied to every decoded frame. | [Hugging Face](https://huggingface.co/Lightricks/LTX-2) |
| **LTX-2 dev** | Pixel-art image-to-video (locked-palette per-frame quantize). | [Hugging Face](https://huggingface.co/Lightricks/LTX-2) |
| **LTX-2.3 22B** | LTX-2.3 (newest LTX) image-to-video, pixel-quantized per frame with a locked palette. | [Hugging Face](https://huggingface.co/Lightricks/LTX-2.3) |
...
| **Wan 2.2** | Pixel-art image-to-video (locked-palette per-frame quantize). | [Hugging Face](https://huggingface.co/Comfy-Org/Wan_2.2_ComfyUI_Repackaged) |
| **Wan 2.2 14B** | Pixel-art image-to-video (locked-palette per-frame quantize). | [Hugging Face](https://huggingface.co/Comfy-Org/Wan_2.2_ComfyUI_Repackaged) |
